...
const char * const BITCOIN_CONF_FILENAME = "cns.conf";
const char * const BITCOIN_PID_FILENAME = "cnsd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CNSCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CNSCore
    // Mac: ~/Library/Application Support/CNSCore
    // Unix: ~/.cnscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CNSCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CNSCore";
#else
    // Unix
    return pathRet / ".cnscore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty cns.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override cns.conf
