...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 CNS irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on cnscentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in cns.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("cns.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in cns.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* CNS features */
    { "cns",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "cns",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "cns",               "gobject",                &gobject,                true,  {} },
    { "cns",               "voteraw",                &voteraw,                true,  {} },
