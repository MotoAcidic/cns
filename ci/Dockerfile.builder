...
# cns_hash
RUN git clone https://github.com/cns/cns_hash
RUN cd cns_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} cns
RUN useradd -u ${USER_ID} -g cns -s /bin/bash -m -d /cns cns
...
RUN mkdir /cns-src && \
  ...
  chown $USER_ID:$GROUP_ID /cns-src && \
