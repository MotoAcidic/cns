...
PACKAGE_VERSION=$($SRC_PATH/src/cnsd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/cnsd $DEB_BIN
cp $SRC_PATH/src/cns-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/cns-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/cnsd.1 $DEB_MAN
cp $SRC_DOC/man/cns-cli.1 $DEB_MAN
cp $SRC_DOC/man/cns-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/cnsd.bash-completion $DEB_CMP/cnsd
cp $SRC_PATH/contrib/cns-cli.bash-completion $DEB_CMP/cns-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/cnsd.1
gzip --best -n $DEB_MAN/cns-cli.1
gzip --best -n $DEB_MAN/cns-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/cnsd $DEB_BIN/cns-cli
