...
// which are part of the CNS repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="cns-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /cns-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "cns-cns/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from cns-cns/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'cns-cns/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from cns-cns/develop"
            }
          }

          if (hasCache) {
            sh "cd /cns-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /cns-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /cns-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /cns-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /cns-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /cns-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /cns-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
